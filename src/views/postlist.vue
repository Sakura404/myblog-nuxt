...
export default {
  ...
  async asyncData({ app }) {
    let postList = await app.$axios.get("/api/posts/").then((res) => {
      if (res.data.code == 10000) {
        return res.data.data;
      }
    });
    let phraseList = await app.$axios.get("/api/phrases/").then((res) => {
      if (res.data.code == 10000) {
        return res.data.data;
      }
    });
    let terms = await app.$axios.get("api/terms/").then((res) => {
      if (res.data.code == 10000) {
        return res.data.data;
      }
    });
    return { terms: terms, phraseList: phraseList, postList: postList };
  },
  head() {
    return {
      meta: [
        {
          hid: "keywords",
          name: "keywords",
          content: this.terms.map((r) => r.name),
        },
      ],
    };
  },
